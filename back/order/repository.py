...
def create_new_orders(db: Session, file_path: str):
    ...
    if not valid_orders:
        logging.warning("No valid orders provieded in the file!")
...
def _check_for_valid_orders(orders: pd.DataFrame, db: Session) -> tuple[list, list]:
    ...
    for index, row in orders.iterrows():
        ...
        if _verify_order_before_add(db, new_order):
            ...
            logging.warning(f"Order from {new_order.aircraft_serial_number} with {new_order.material_part_number} skipped due to validation error!")
...
def _verify_order_before_add(db: Session, order: Order) -> bool:
    ...
    if not all([order.material_part_number, order.aircraft_serial_number, order.arrival_date, order.status]):
        logging.warning("Missing information about order!")
        return False
    ...
    if not str(order.status).lower() in VALID_STATUS:
        logging.warning(f"Status is {order} but should be one of {VALID_STATUS}")
        return False

    aircraft = db.query(Aircraft).filter(Aircraft.serial_number == order.aircraft_serial_number).first()
    if not aircraft:
        logging.warning(f"Aircraft {order.aircraft_serial_number} is not in the database. Cannot make order for it!")
        return False

    material = db.query(Material).filter(Material.part_number == order.material_part_number).first()
    if not material:
        logging.warning(f"Material {order.material_part_number} is not in the database. Cannot make order with it!")
        return False
    ...
    if existing_order:
        logging.warning(f"This order with {order.aircraft_serial_number} and {order.material_part_number} from {order.arrival_date} is already in the database!") 
        return False
